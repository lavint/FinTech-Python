# Automation with Python lists and loops
![automation.jpg](Images/automation.jpg)

## PyBank


Create a Python script that analyzes the records in [budget_data.csv](PyBank/Resources/budget_data.csv)  and calculate each of the following:
...
Analyze the business's financial performance by cross-referencing the sales data with the internal menu data to figure out revenues and costs for the year.

Also, analyze how well the business did on a per-product basis (as there are several choices of ramen) in order to better understand which products are doing well, which are doing poorly, and, ultimately, which products may need to be removed or changed.
...
Break down the tasks into discrete mini-objectives. This will be a *much* better course of action than attempting to Google search for a miracle.
